"""SQLite schema for Listening tests, question sections and transcripts."""
from pathlib import Path
import sqlite3
from reading_store import ROOT

DEFAULT_DB = ROOT / 'listening_books.db'
SCHEMA = '''
...
CREATE TABLE IF NOT EXISTS parts (
 ...
 part_number INTEGER NOT NULL, title TEXT NOT NULL,
 source_pages TEXT NOT NULL, UNIQUE(test_id,part_number));
CREATE TABLE IF NOT EXISTS sections (
 ...
 source_pages TEXT NOT NULL, UNIQUE(part_id,position));
...
CREATE TABLE IF NOT EXISTS transcripts (
 ...
 content TEXT NOT NULL, segments_json TEXT NOT NULL, source_pages TEXT NOT NULL);
...
CREATE VIEW IF NOT EXISTS question_details AS
 SELECT t.book_number,t.test_number,p.part_number,p.title part_title,
 ...
 JOIN sections s ON s.id=q.section_id JOIN parts p ON p.id=s.part_id
...
def connect(path=DEFAULT_DB, readonly=False):
    ...
    if readonly:
        ...
        if 'audio_url' in [r[1] for r in db.execute('PRAGMA table_info(parts)')]:
            db.execute('DROP VIEW IF EXISTS question_details')
            db.execute('ALTER TABLE parts DROP COLUMN audio_url')
